...
lfq_trainer.py
功能说明：
本模块为 LFQTokenizer 项目的训练脚本，负责读取配置、初始化 wandb、数据模块、模型、训练器并执行训练。
"""
...
from vqtokenizer.nn.lfq_backbone import LFQTokenizer
...
def main(cfg: DictConfig) -> None:
    ...
    # Create model (add temperature, commitment_cost if present)
    model = LFQTokenizer(
        input_dim=input_dim,
        ...
        commitment_cost=getattr(cfg.model, "commitment_cost", 0.25),
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LFQTokenizer Trainer: Training for protein structure LFQ models.")
    parser.add_argument(
        "-c",
        "--config",
        type=str,
    ...
    if args.config:
        ...
        if len(sys.argv) == 1:
            if not os.path.exists(config_path):
                ...
                sys.exit(1)
            print("[ERROR] Please run with:\n  python lfq_trainer.py --config-path config --config-name train.yml")
            sys.exit(1)
        else:
